document.addEventListener("DOMContentLoaded", function () {
    ...
    function checkEmailConfirmationCode() {
        ...
        if (userCode !== emailConfirmCode) {
            emailConfirmMsg.innerHTML = '<span>인증번호가 잘못되었습니다</span>';
            emailConfirmMsg.style.color = '#FA3E3E';
            emailConfirmMsg.style.fontWeight = 'bold';
            emailConfirmMsg.style.fontSize = '10px';
            submitBtn.disabled = true;
        } else {
            emailConfirmMsg.innerHTML = '<span>인증번호가 일치합니다</span>';
    ...
    function needFilmAll() {
        let email = document.querySelector("#email").value;
        let emailConfirm = document.querySelector(".memailconfirmTxt").value;
